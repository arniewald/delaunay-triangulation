...
def mc_add_barycenters(data,labels,dim,sample,out_hull,tri,tri_errors,run_params):
    """
...
def mc_delaunayization2(data,sample,labels,dim,dim_labels,run_params):
    """
    Performs Delaunay triangulation, computes barycentric coordinates, estimates labels and estimates error with multiple labels.
...
def mc_movepoints_step(data,sample,tri,out_hull,err,run_params):
    al = run_params['al']
...
def mc_add_barycenters_step(data,labels,dim,dim_labels,sample,rem,out_hull,tri,tri_errors,run_params,t):
    added = []
    ...
    if (t+1)%bc_time==0:
        ...
        tri_errors = mc_mean_training_error(data,dim,sample,rem,bc,tri,e)
        print('Errors calculated')
        data, labels, sample, out_hull, added = mc_add_barycenters(data,labels,dim,sample,out_hull,tri,tri_errors,run_params)
        print('Points added: ',len(added))
        out_hull = [int(x) for x in out_hull]
        tri, bc, e, labels[sample,:], err = mc_delaunayization2(data,sample,labels,dim,dim_labels,run_params)
        end = time()
        print('Time to add barycenters: '+str(end-start))
    return data, labels, sample, out_hull, added, tri, bc, e, labels[sample,:], err

def mc_movepoints2(data,labels,sample,rem,out_hull,dim,dim_labels,run_params,it,filename = "",test_data=[],real=[]):
    """
    Performs the estimation of labels and movement of points as many times as indicated.
    Also writes, for each iteration: the sum of estimated errors, the sum of the squared residuals, the variance of the estimated
    error and the real error (only if function f applied).
    ...
    for i in range(it):
        ...
        try:
            tri, bc, e, labels[sample,:], err = mc_delaunayization2(data,sample,labels,dim,dim_labels,run_params)
            #data, labels, sample, out_hull, added, tri, bc, e, labels[sample,:], err = mc_add_barycenters_step(data,labels,dim,dim_labels,sample,rem,out_hull,tri,tri_errors,run_params,i)
            ...
            err_dict['rerrs'].append(mc_compute_real_error(test_data, dim, tri, labels[sample], real))
            print(i,err_dict['avs'][i],err_dict['sigmas'][i],err_dict['evars'][i],err_dict['rerrs'][i])

            #We move the points
            data = mc_movepoints_step(data,sample,tri,out_hull,err,run_params)

        except Exception as e:
            print("Exception at time ",i,":",e)
            break

    tri, _, e, labels[sample,:], err = mc_delaunayization2(data,sample,labels,dim,dim_labels,run_params)
    print("Total final data: ",len(data))
    
    

    return data, labels, sample, added, tri, e, err_dict

def save_results(data_name,data,labels,sample,rem,added,tri,dim,err_dict):
    ...
    data_plot = plot_data(data,labels,sample,rem,added,tri,dim)
    err_csv = pd.DataFrame.from_dict(err_dict)
    err_csv.to_csv(errors_path+'\\'+filename+'.csv')
    data_plot.savefig(media_path+'\\'+filename)
